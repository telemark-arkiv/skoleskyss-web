...
module.exports = document => {
  return new Promise(async (resolve, reject) => {
    ...
    logger('info', ['check-duplicate-applications', 'applicantId', applicantId])
    try {
      const result = await axios.post(url, query)
      const duplicates = result.data.length
      logger('info', ['check-duplicate-applications', 'applicantId', applicantId, 'duplicates', duplicates])
      resolve(duplicates > 0)
    } catch (error) {
      logger('error', ['check-duplicate-applications', 'applicantId', applicantId, error])
      reject(error)
    }
  })
}
